...
Application::Application(void)
{
    ...
    m_Window = glfwCreateWindow(1366, 768, "This is a title", NULL, NULL);
    if (!m_Window)
    {
        throw std::runtime_error("Cannot create window");
    }

    glfwSetKeyCallback(m_Window, Input_KeyCallback);
...
void Application::Run(void)
{
    ...
    TextureHandle atlas{"data/atlas.png"};
    ...
    registry.emplace<VelocityComponent>(entity, 1.0f, 1.0f);
    registry.emplace<SpriteAnimation>(entity, animations["skelet_run_anim"]);
    ...
    while (!glfwWindowShouldClose(m_Window))
    {
        ...
        registry.view<PositionComponent, const VelocityComponent>().each(
        ...
        m4 projection = m4::Orthographic(0.0f, (f32)windowWidth, (f32)windowHeight, 0.0f,
                                         -1.0f, 1.0f);
        ...
        {
            registry.view<const PositionComponent, SpriteAnimation>().each(
                [&renderer, deltaTime](const auto &pos, auto &ani)
                {
                    ani.Update(deltaTime);
                    renderer.DrawTexture(v2{pos.x, pos.y}, ani.GetFrameRect());
                });
        }
        renderer.EndDraw();
